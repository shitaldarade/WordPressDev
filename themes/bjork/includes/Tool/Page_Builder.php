...
use WebManDesign\Bjork\Header\Body_Class;
...
class Page_Builder implements Component_Interface {
	...
	public static function init() {
		...
		// Processing
			...
			// Filters
				...
				add_filter( 'pre/bjork/entry/media/display', __CLASS__ . '::return_empty_string_when_enabled', 100 );

				add_filter( 'bjork/content/show_primary_title', __CLASS__ . '::return_false_when_enabled', 20 );

				add_filter( 'theme_templates', __CLASS__ . '::remove_page_template' );

				add_filter( 'bjork/tool/page_builder/is_enabled', __CLASS__ . '::automatic_check' );
	...
	public static function options( array $options ): array {
		...
		// Processing
			...
			$options[ 940 . 'page_builders' ] = array(
				'id'             => 'page_builders',
				'type'           => 'section',
				'create_section' => esc_html_x( 'Page builders', 'Customizer section title.', 'bjork' ),
			...
			$options[ 940 . 'page_builders' . 100 ] = array(
				...
				'label'       => esc_html__( 'Enable "Page builder" page/post template', 'bjork' ),
				'description' => esc_html__( 'Use this page/post template when building a content with your page builder plugin.', 'bjork' ) . ' ' . esc_html__( 'You can tweak the desired layout of this template below.', 'bjork' ) . '<br>(<a href="https://support.webmandesign.eu/page-template/" target="_blank"  rel="noopener noreferrer">' . esc_html__( 'Open a page/post template instructions in new window &rarr;', 'bjork' ) . '</a>)',
			...
			$options[ 940 . 'page_builders' . 110 ] = array(
				...
				'label'       => esc_html__( 'Page builder layout', 'bjork' ),
				'description' => esc_html__( 'Tweaks content area layout when using "Page builder" template.', 'bjork' ) . ' ' . esc_html__( 'As every page builder plugin works differently, set this according to your needs.', 'bjork' ),
				'default'     => ( $is_no_padding_default ) ? ( 'no-padding' ) : ( 'full-width' ),
				'choices'     => array(
					'full-width' => esc_html__( 'Full width content area, no padding', 'bjork' ),
					'no-padding' => esc_html__( 'Keep content area width, just remove padding', 'bjork' ),
				),
			);
	...
	public static function is_enabled( $body_classes = array() ): bool {
		...
		// Variables
			...
			$check_body_class = stripos( implode( ' ', Body_Class::get_body_class( $body_classes ) ), 'template-builder' );
		...
		// Output
			 ...
			 */
			return (bool) apply_filters( 'bjork/tool/page_builder/is_enabled', (bool) $check_body_class );
		...
		// Processing
			...
			if ( ! Mod::get( 'page_builder_template' ) ) {
				foreach ( $post_templates as $file => $name ) {
					if ( 0 === stripos( $file, 'templates/builder' ) ) {
