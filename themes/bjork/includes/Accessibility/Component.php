...
namespace WebManDesign\Bjork\Accessibility;

use WebManDesign\Bjork\Component_Interface;
use WebManDesign\Bjork\Assets;
use WebManDesign\Bjork\Header;
...
class Component implements Component_Interface {
	...
	public static function init() {
		...
		// Processing
			...
			// Actions
				...
				add_action( 'wp_enqueue_scripts', __CLASS__ . '::enqueue_inline_skip_link_focus_fix', BJORK_ENQUEUE_PRIORITY );

				add_action( 'tha_body_top', __CLASS__ . '::anchor_top_of_page', -10 );
				add_action( 'bjork/site_info/after', __CLASS__ . '::to_the_top' );
			...
			// Filters
				...
				add_filter( 'walker_nav_menu_start_el', __CLASS__ . '::nav_menu_item_label', 10, 4 );
						...
						'a11yMenuConfig' => array(
							...
							'button_attributes' => array(
								...
								'aria-label' => array(
									...
									'collapse' => esc_html__( 'Collapse menu: %s', 'bjork' ),
									/* translators: %s: menu item label. */
									'expand'   => esc_html__( 'Expand menu: %s', 'bjork' ),
								),
							),
						),
					),
	 ...
	 * @return  void
	 */
	public static function to_the_top() {

		// Output

			echo '<a href="#top" class="back-to-top">' . esc_html__( 'To the top &uarr;', 'bjork' ) . '</a>';
	...
	public static function link_skip_to( string $id = 'content', string $text = '', string $class = '', string $html = '%s' ): string {
		...
		// Pre
			 ...
			 * @param  string $html   Output html, use "%s" for actual link output.
			 */
			$pre = apply_filters( 'pre/bjork/accessibility/link_skip_to', false, $id, $text, $class, $html );
		...
		// Processing
			...
			if ( empty( $text ) ) {
				$text = __( 'Skip to main content', 'bjork' );
			}
		...
		// Output
			...
			echo
				...
				. self::link_skip_to( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
					'site-navigation',
					esc_html__( 'Skip back to main navigation', 'bjork' )
				)
				. '</div>';
