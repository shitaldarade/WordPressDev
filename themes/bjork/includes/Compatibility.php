...
class Bjork_Compatibility {
	...
	public static function get_message() {
		...
		// Processing
			...
			if ( version_compare( $GLOBALS['wp_version'], BJORK_WP_VERSION, '<' ) ) {
				$output[] = sprintf(
					/* translators: 1: required WP version number, 2: available WP version number */
					esc_html__( 'This theme requires at least WordPress version %1$s. You are running version %2$s.', 'bjork' ),
					BJORK_WP_VERSION,
					$GLOBALS['wp_version']
				);
			}

			if ( version_compare( PHP_VERSION, BJORK_PHP_VERSION, '<' ) ) {
				$output[] = sprintf(
					/* translators: 1: required PHP version number, 2: available PHP version number */
					esc_html__( 'This theme requires at least PHP version %1$s. You are running version %2$s.', 'bjork' ),
					BJORK_PHP_VERSION,
					PHP_VERSION
				);
			}
			...
			if ( ! empty( $output ) ) {
				$output[] = __( 'Please upgrade and try again.', 'bjork' );
			}
