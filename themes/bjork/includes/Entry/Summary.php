...
class Summary implements Component_Interface {
		...
		// Requirements check
			...
			if ( post_password_required( $post_id ) ) {
				if ( ! is_single( $post_id ) ) {
					return
						esc_html__( 'This content is password protected.', 'bjork' )
						. ' <a href="' . esc_url( get_permalink() ) . '">'
						. esc_html__( 'Enter the password to view it.', 'bjork' )
						. '</a>';
				}
				return '';
			}
	...
	public static function get_continue_reading_html( $post = null, string $context = '' ): string {
		...
		// Pre
			...
			/**
			 * Bypass filter for WebManDesign\Bjork\Entry\Summary::get_continue_reading_html().
			 *
			 ...
			 */
			$pre = apply_filters( 'pre/bjork/entry/summary/get_continue_reading_html', false, $post, $context );
		...
		// Output
			 ...
			 * @param  string           $context  Optional context.
			 */
			return (string) apply_filters( 'bjork/entry/summary/get_continue_reading_html', $html, $post, $context );
