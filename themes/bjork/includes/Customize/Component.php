...
class Component implements Component_Interface {
	...
	public static function init() {
		...
		// Processing
			...
			// Actions
				...
				add_action( 'bjork/customize/render/option', __NAMESPACE__ . '\Control::add_option', 10, 2 );
	...
	public static function render( WP_Customize_Manager $wp_customize ) {
		...
		// Pre
			...
			/**
			 * Bypass filter for WebManDesign\Bjork\Customize\Component::register().
			 *
			 ...
			 */
			$pre = apply_filters( 'pre/bjork/customize/render', null, $wp_customize );
		...
		// Variables
			...
			$priority = absint(
				 ...
				 * @param  int $priority
				 */
				apply_filters( 'bjork/customize/render/priority', 0 )
			);
		...
		// Processing
			...
			foreach ( $options as $key => $option ) {
				if ( isset( $option['type'] ) ) {
					...
					// Preset option args.
					if ( ! isset( $option['id'] ) ) {
						$option['id'] = 'bjork' . '_key_' . sanitize_title( $key );
					}
					...
					if ( isset( $option['in_panel'] ) ) {
						 ...
						 * @param  array                $options
						 */
						$panel_args = (array) apply_filters( 'bjork/customize/render/panel_args', $panel_args, $option, $wp_customize, $options );
					...
					if ( ! in_array( $option['type'], array( 'panel', 'section' ) ) ) {
						 ...
						 * @param  array                $options
						 */
						do_action( 'bjork/customize/render/option', $option, $wp_customize, $options );
					}
				}
			}
