...
class Styles implements Component_Interface {
	...
	public static function init() {
		...
		// Processing
			...
			// Actions
				...
				add_action( 'wp_enqueue_scripts', __CLASS__ . '::enqueue', BJORK_ENQUEUE_PRIORITY );

				add_action( 'wp_enqueue_scripts', __CLASS__ . '::register_inline', 0 );
				add_action( 'wp_enqueue_scripts', __CLASS__ . '::enqueue_inline', BJORK_ENQUEUE_PRIORITY + 9 );

				add_action( 'wp_enqueue_scripts', __CLASS__ . '::enqueue_child_theme', BJORK_ENQUEUE_PRIORITY + 9 );

				add_action( 'wp_head', __CLASS__ . '::preload' );

				add_action( 'tha_content_top',     __CLASS__ . '::print', 0 );
				add_action( 'tha_sidebars_before', __CLASS__ . '::print', 0 );
				add_action( 'tha_comments_before', __CLASS__ . '::print', 0 );
				add_action( 'wp_footer',           __CLASS__ . '::print', 0 );
	...
	public static function get_css_files(): array {
		...
		// Processing
			...
			$css_files = array(
				...
				'bjork-widgets' => array(
					'src'      => get_theme_file_uri( 'assets/css/widgets.css' ),
					'add_data' => array(
						'rtl'      => 'replace',
						'precache' => true,
					),
			 ...
			 */
			$css_files = (array) apply_filters( 'bjork/assets/styles/get_css_files', $css_files );
	...
	public static function print() {
		...
		// Processing
			...
			if ( doing_action( 'tha_content_top' ) ) {
				Factory::print_styles( 'bjork-content', 'bjork-blocks' );
			}

			// Comments styles.
			if ( doing_action( 'tha_comments_before' ) ) {
				Factory::print_styles( 'bjork-comments' );
			}

			// Sidebars and widgets styles.
			if ( doing_action( 'tha_sidebars_before' ) ) {
				Factory::print_styles( 'bjork-widgets' );
			}
	 ...
	 * Use the `bjork` handle in `wp_add_inline_style`.
	 *
